...
int main() {
	CvSize size640x480 = cvSize(640, 360);			// use a 640 x 480 size for all windows, also make sure your webcam is set to 640x480 !!
	...
	while(1) {								// for each frame . . .
		p_imgCaptured = cvQueryFrame(p_capWebcam);		// get frame from webcam
        p_imgOriginal = cvCreateImage(size640x480,			// 640 x 480 pixels (CvSize struct from earlier)
								   IPL_DEPTH_8U,		// 8-bit color depth
								   3);					// 1 channel (grayscale), if this was a color image, use 3
        cvResize(p_imgCaptured, p_imgOriginal, CV_INTER_AREA);
        ...
        CvScalar scalar1 = CV_RGB(175,0,0);
        CvScalar scalar2 = CV_RGB(256,100,100);
		cvInRangeS(p_imgOriginal,				// function input
				   scalar1,			// min filtering value (if color is greater than or equal to this)
				   scalar2,			// max filtering value (if color is less than this)
